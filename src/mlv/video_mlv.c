...
int mlvSaveAVFrame(mlvObject_t * video, FILE * output_mlv, int export_audio, int export_mode, uint32_t frame_start, uint32_t frame_end, uint32_t frame_index)
{
    ...
    uint32_t frame_size_unpacked = video->RAWI.xRes * video->RAWI.yRes * 2;
    ...
    uint8_t * frame_buf = calloc(frame_size, 1);
    if(!frame_buf)
    {
        DEBUG( printf("\nCould not allocate memory for VIDF frame data\n"); )
        free(block_buf);
        return 1;
    }
    ...
    if(export_mode == MLV_AVERAGED_FRAME) // average all frames to one dark frame
    {
        ...
        else if(vidf_hdr.frameNumber == frame_end - frame_start + 1)
        {
