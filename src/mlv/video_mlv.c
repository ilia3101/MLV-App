...
int saveMlvAVFrame(mlvObject_t * video, FILE * output_mlv, int export_audio, int export_mode, uint32_t frame_start, uint32_t frame_end, uint32_t frame_index, uint32_t * avg_buf, char * error_message)
{
    ...
    if(!(frame_start - frame_index - 1) && export_audio && export_mode < MLV_AVERAGED_FRAME )
    {
        ...
        uint64_t audio_start_offset = (uint64_t)( (double)(getMlvAudioChannels(video) * getMlvSampleRate(video) * getMlvAudioBitsPerSample(video) * (frame_start - 1)) / (double)getMlvFramerate(video) );
        uint64_t cut_audio_size_rough = (uint64_t)( (double)(getMlvAudioChannels(video) * getMlvSampleRate(video) * getMlvAudioBitsPerSample(video) * (frame_end - frame_start + 1)) / (double)getMlvFramerate(video) );
