...
/*
  ...
  Parameters:
    ...
    Input, float TDATA(5), the nodes associated with the basis function.
    The entries of TDATA are assumed to be distinct and increasing.

    Input, float TVAL, a point at which the B spline basis function is
    to be evaluated.

    Output, float BASIS_FUNCTION_B_VAL, the value of the function at TVAL.
*/
{
# define NDATA 5
...
/*
  ...
  Parameters:
    ...
    Input, float BETA1, the skew or bias parameter.
    BETA1 = 1 for no skew or bias.

    Input, float BETA2, the tension parameter.
    ...
    Output, float BASIS_FUNCTION_BETA_VAL, the value of the function at TVAL.
*/
{
# define NDATA 5
...
/*
  ...
  Parameters:
    ...
    Output, float BASIS_MATRIX_B_UNI[4*4], the basis matrix.
*/
{
  int i;
  int j;
  float *mbasis;
  float mbasis_save[4*4] = {
    -1.0 / 6.0, 
     3.0 / 6.0, 
    -3.0 / 6.0, 
     1.0 / 6.0,
  ...
  mbasis = ( float * ) malloc ( 4 * 4 * sizeof ( float ) );
...
/*
  ...
  Parameters:
    ...
    Output, float BASIS_MATRIX_BETA_UNI[4*4], the basis matrix.
*/
{
  float delta;
  int i;
...
/*
  ...
  Parameters:
    ...
    Output, float BASIS_MATRIX_BEZIER[4*4], the basis matrix.
*/
{
  float *mbasis;
...
/*
  ...
  Parameters:
    ...
    Output, float BASIS_MATRIX_HERMITE[4*4], the basis matrix.
*/
{
  float *mbasis;
...
/*
  ...
  Parameters:
    ...
    Input, float ALPHA, BETA.
    ALPHA = || P2 - P1 || / ( || P3 - P2 || + || P2 - P1 || )
    BETA  = || P3 - P2 || / ( || P4 - P3 || + || P3 - P2 || ).

    Output, float BASIS_MATRIX_OVERHAUSER_NONUNI[4*4], the basis matrix.
*/
{
  float *mbasis;
...
/*
  ...
  Parameters:
    ...
    Output, float BASIS_MATRIX_OVERHAUSER_NUL[3*3], the basis matrix.
*/
{
  float *mbasis;

  mbasis = ( float * ) malloc ( 3 * 3 * sizeof ( float ) );
...
/*
  ...
  Parameters:
    ...
    Output, float BASIS_MATRIX_OVERHAUSER_NUR[3*3], the basis matrix.
*/
{
  float *mbasis;
...
/*
  ...
  Parameters:
    ...
    Output, float BASIS_MATRIX_OVERHASUER_UNI[4*4], the basis matrix.
*/
{
  float *mbasis;
...
/*
  ...
  Parameters:
    ...
    Output, float BASIS_MATRIX_OVERHASUER_UNI_L[3*3], the basis matrix.
*/
{
  float *mbasis;
...
/*
  ...
  Parameters:
    ...
    Output, float BASIS_MATRIX_OVERHASUER_UNI_R[3*3], the basis matrix.
*/
{
  float *mbasis;
...
/*
  ...
  Parameters:
    ...
    Input, float MBASIS[N*N], the basis matrix.

    Input, int NDATA, the dimension of the vectors TDATA and YDATA.

    Input, float TDATA[NDATA], the abscissa values.  This routine
    assumes that the TDATA values are uniformly spaced, with an
    increment of 1.0.

    Input, float YDATA[NDATA], the data values to be interpolated or
    approximated.

    Input, float TVAL, the value of T at which the spline is to be
    evaluated.

    Output, float BASIS_MATRIX_TMP, the value of the spline at TVAL.
*/
{
  float arg;
  int first;
  ...
  tvec = ( float * ) malloc ( n * sizeof ( float ) );
...
/*
  ...
  Parameters:
    ...
    Input, float T, the point at which the Bezier curve should
    be evaluated.  The best results are obtained within the interval
    [0,1] but T may be anywhere.

    Input, float XCON[0:N], YCON[0:N], the X and Y coordinates
    ...
    Output, float *XVAL, *YVAL, the X and Y coordinates of the point
...
/*
  ...
  Parameters:
    ...
    Input, float X, the point at which the Bezier function should
    be evaluated.  The best results are obtained within the interval
    [A,B] but X may be anywhere.

    Input, float A, B, the interval over which the Bezier function
    ...
    Input, float Y[0:N], a set of data defining the Y coordinates
    of the control points.

    Output, float BEZ_VAL, the value of the Bezier function at X.
*/
{
  float *bval;
  int i;
...
/*
  ...
  Parameters:
    ...
    Input, float A, B, the endpoints of the interval on which the
    approximant is based.  A and B should not be equal.

    Input, float YDATA[0:N], the data values at N+1 equally spaced points
    in [A,B].  If N = 0, then the evaluation point should be 0.5 * ( A + B).
    Otherwise, evaluation point I should be ( (N-I)*A + I*B ) / N ).

    Input, float XVAL, the point at which the Bernstein polynomial
    approximant is to be evaluated.  XVAL does not have to lie in the
    interval [A,B].

    Output, float BPAB_APPROX, the value of the Bernstein polynomial approximant
...
/*
  ...
  Parameters:
    ...
    Output, float BP01[0:N], the values of the N+1 Bernstein basis
    polynomials at X.
*/
{
  float *bern;
  ...
  bern = ( float * ) malloc ( ( n + 1 ) * sizeof ( float ) );
...
/*
  ...
  Parameters:
    ...
    Input, float X, the point at which the polynomials are to be
    evaluated.  X need not lie in the interval [A,B].

    Output, float BERN[0:N], the values of the N+1 Bernstein basis
    polynomials at X.
*/
{
  float *bern;
...
/*
  ...
  Parameters:
    ...
    Input, float X1, X2, the endpoints of the interval of
    definition of the cubic.  X1 and X2 must be distinct.

    Input, float F1, F2, the values of the function at X1 and
    X2, respectively.

    Input, float D1, D2, the derivative values at X1 and
    X2, respectively.

    Input, int NE, the number of evaluation points.

    Input, float XE[NE], the points at which the function is to
    be evaluated.  If any of the XE are outside the interval
    [X1,X2], a warning error is returned in NEXT.

    Output, float FE[NE], the value of the cubic function
    at the points XE.
...
/*
  ...
  Parameters:
    ...
    Input, float A[3*N], the D3 matrix.

    Input, float X[N], the vector to be multiplied by A.

    Output, float D3_MXV[N], the product A * x.
*/
{
  float *b;
  int i;
...
/*
  ...
  Parameters:
    ...
    Output, float D3_UNIFORM[3*N], the D3 matrix.
*/
{
  float *a;
  int i;
  ...
  a = ( float * ) malloc ( 3 * n * sizeof ( float ) );
...
/*
  ...
  Parameters:
    ...
    Input, float XTAB[NTAB], the X values at which data was taken.
    These values must be distinct.

    Input, float YTAB[NTAB], the corresponding Y values.

    Output, float DIFTAB[NTAB], the divided difference coefficients
...
/*
  ...
  Parameters:
    ...
    Input, float XTAB[NTAB], the X values upon which the
    divided difference polynomial is based.

    Input, float DIFTAB[NTAB], the divided difference table.

    Input, float XVAL, a value of X at which the polynomial
    is to be evaluated.

    Output, float DIF_VAL, the value of the polynomial at XVAL.
*/
{
  int i;
  float value;
...
/*
  ...
  Parameters:
    ...
    Input, float X[POINT_NUM], the abscissas of the data points.
    At least NTERMS of the values in X must be distinct.

    Input, float F[POINT_NUM], the data values at the points X(*).

    Input, float W[POINT_NUM], the weights associated with
    ...
    Output, float B[NTERMS], C[NTERMS], D[NTERMS], are quantities
...
*/
  ...
  s = ( float * ) malloc ( nterms * sizeof ( float ) );
/*
...
*/
  pjm1 = ( float * ) malloc ( point_num * sizeof ( float ) );
  pj = ( float * ) malloc ( point_num * sizeof ( float ) );
...
/*
  ...
  Parameters:
    ...
    Input, float B[NTERMS], C[NTERMS], D[NTERMS], the information
    computed by R8POLY_LS_SET.

    Input, float X, the point at which the least squares polynomial
    is to be evaluated.

    Output, float LEAST_VAL, the value of the least squares
    polynomial at X.
*/
{
  int i;
...
/*
  ...
  Parameters:
    ...
    Output, float *PX, *PXP, the value and derivative of the least
...
/*
  ...
  Parameters:
    ...
    Input, float XTAB[NTAB], the X data.  The values in XTAB
    should be distinct, and in increasing order.

    Input, float YTAB[NTAB], the Y data values corresponding
    ...
    Output, float PTAB[NTAB], the value of the least squares polynomial
    at the points XTAB(1:NTAB).

    Output, float B[1:NDEG], C[0:NDEG], D[2:NDEG], arrays containing
    data about the polynomial.

    Output, float *EPS, the root-mean-square discrepancy of the
    polynomial fit.
...
*/
{
  ...
  *ierror = 0;
  ztab = ( float * ) malloc ( 2 * ntab * sizeof ( float ) );
/*
  Check NDEG.
*/
  if ( ndeg < 1 )
...
*/
  ...
  rn0 = ntab;
  c[0] = y_sum / ( float ) ( ntab );

  for ( i = 0; i < ntab; i++ )
  {
    ptab[i] = y_sum / ( float ) ( ntab );
  }
  ...
  if ( ndeg == 0 )
  {
    *eps = 0.0;
    for ( i = 0; i < ntab; i++ )
    {
      *eps = *eps + pow ( ( y_sum / ( float ) ( ntab ) - ytab[i] ), 2 );
    }

    *eps = sqrt ( *eps / ( float ) ( ntab ) );
    free ( ztab );
    return;
  }
/*
...
*/
  ...
  b[1+B_OFFSET] = ztab[0] / ( float ) ( ntab );
...
/*
  ...
  Parameters:
    ...
    Input, float B[1:NDEG], C[0:NDEG], D[2:NDEG], arrays defined by
    LEAST_SET, and needed to evaluate the polynomial.

    Output, float LEAST_VALPOLD, the value of the polynomial at X.
*/
{
  int B_OFFSET = -1;
  int D_OFFSET = -2;
...
/*
  ...
  Parameters:
    ...
    Input, float YDATA[NDIM*NDATA], the data points corresponding to
    the abscissas.
    ...
    Output, float YVAL[NDIM], the value of the parabolic interpolant
    at TVAL.
*/
{
  float dif1;
...
/*
  ...
  Parameters:
    ...
    Output, float R8GE_FS_NEW[N], the solution of the linear system.
*/
{
  int i;
  int ipiv;
...
/*
  ...
  Parameters:
    ...
    Output, float R8VEC_INDICATOR_NEW[N], the array.
*/
{
  float *a;
  int i;
...
/*
  ...
  Parameters:
    ...
    Output, float R8VEC_UNIFORM_NEW[N], the vector of pseudorandom values.
*/
{
  int i;
...
/*
  ...
  Parameters:
    ...
    Input, float TDATA[NDATA], the abscissas of the data.

    Input, float YDATA[NDATA], the data values.

    Input, float TVAL, a point at which the spline is to be evaluated.

    Output, float SPLINE_B_VAL, the value of the function at TVAL.
*/
{
  float bval;
  int left;
...
/*
  ...
  Parameters:
    ...
    Output, float SPLINE_BETA_VAL, the value of the function at TVAL.
*/
{
  float a;
  float b;
...
/*
  ...
  Parameters:
    ...
    Input, float TDATA[NDATA-1], the breakpoints.  The values of TDATA should
    be distinct and increasing.

    Input, float YDATA[NDATA], the values of the spline in the intervals
    defined by the breakpoints.

    Input, float TVAL, the point at which the spline is to be evaluated.

    Output, float *SPLINE_CONSTANT_VAL, the value of the spline at TVAL.
*/
{
  int i;
...
/*
  ...
  Parameters:
    ...
    Output, float SPLINE_CUBIC_SET[N], the second derivatives
    of the cubic spline.
*/
{
  float *a1;
...
*/
  ...
  a4 = ( float * ) malloc ( n * sizeof ( float ) );
  a5 = ( float * ) malloc ( n * sizeof ( float ) );
  b = ( float * ) malloc ( n * sizeof ( float ) );
...
*/
  ...
  {
    ypp = ( float * ) malloc ( 2 * sizeof ( float ) );
...
/*
  ...
  Parameters:
    ...
    Output, float *YVAL, *YPVAL, *YPPVAL, the value of the spline, and
...
/*
  ...
  Parameters:
    ...
    Input, float TDATA[NDATA], the abscissas of the data points.
    The entries of TDATA are assumed to be strictly increasing.

    Input, float Y[NDATA], YP[NDATA], the value of the
    function and its derivative at TDATA(1:NDATA).

    Output, float SPLINE_HERMITE_SET[4*NDATA], the coefficients of
...
*/
{
  ...
  c = ( float * ) malloc ( 4 * ndata * sizeof ( float ) );
...
/*
  ...
  Parameters:
    ...
    Input, float C[4*NDATA], the coefficient data computed by
    SPLINE_HERMITE_SET.

    Input, float TVAL, the point where the interpolant is to
    be evaluated.

    Output, float *SVAL, *SPVAL, the value of the interpolant
...
float spline_linear_int ( int ndata, float tdata[], float ydata[],
  float a, float b )
...
/*
  ...
  Parameters:
    ...
    Input, float TDATA[NDATA], YDATA[NDATA], the values of the independent
    and dependent variables at the data points.  The values of TDATA should
    be distinct and increasing.

    Input, float A, B, the interval over which the integral is desired.

    Output, float SPLINE_LINEAR_INT, the value of the integral.
*/
{
  float a_copy;
  int a_left;
...
void spline_linear_intset ( int n, float int_x[], float int_v[],
...
/*
  ...
  Parameters:
    ...
    Input, float INT_X[N+1], the points that define the intervals.
    Interval I lies between INT_X(I) and INT_X(I+1).

    Input, float INT_V[N], the desired value of the integral of the
    linear spline over each interval.

    Output, float DATA_X[N], DATA_Y[N], the values of the independent
...
/*
  ...
  Parameters:
    ...
    Input, float TVAL, the value where the spline is to
    be evaluated.

    Output, float SPLINE_OVERHAUSER_NONUNI_VAL, the value of the
    spline at TVAL.
*/
{
  float alpha;
...
/*
  ...
  Parameters:
    ...
    Output, float SPLINE_OVERHAUSER_UNI_VAL, the value of the spline at TVAL.
*/
{
  int left;
  float *mbasis;
...
void spline_overhauser_val ( int ndim, int ndata, float tdata[],
  float ydata[], float tval, float yval[] )
...
/*
  ...
  Parameters:
    ...
    Input, float TVAL, the abscissa value at which the spline
    is to be evaluated.  Normally, TDATA[0] <= TVAL <= T[NDATA-1], and 
    the data will be interpolated.  For TVAL outside this range, 
    extrapolation will be used.

    Output, float YVAL[NDIM], the value of the spline at TVAL.
*/
{
  int i;
  int left;
...
*/
  yl = ( float * ) malloc ( ndim * sizeof ( float ) );
  yr = ( float * ) malloc ( ndim * sizeof ( float ) );
...
/*
  ...
  Parameters:
    ...
    Input, float X[N], the strictly increasing independent
    variable values.

    Input, float F[N], dependent variable values to be interpolated.  This
    ...
    Output, float D[N], the derivative values at the
...
void spline_pchip_val ( int n, float x[], float f[], float d[],
  int ne, float xe[], float fe[] )
...
/*
  ...
  Parameters:
    ...
    Input, float F[N], the function values.

    Input, float D[N], the derivative values.

    Input, int NE, the number of evaluation points.

    Input, float XE[NE], points at which the function is to
    be evaluated.

    Output, float FE[NE], the values of the cubic Hermite
    function at XE.
*/
{
  int i;
