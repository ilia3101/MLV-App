...
/* Black and white 1 bit BMP write function */
void write_bmp2_1(imagestruct image, char * imagename)
{
    uint32_t paddedwidth = pad(image.width * 3, 4);
    /* Open/create file */
    FILE * file = fopen(imagename,"wb");
    /* Create header and write it to file */
    bitmapfileheader header1 = { 0x4D42, 54, 0, 0, 26 };
    fwrite(&header1, 1, sizeof(bitmapfileheader), file);
    bitmap2header header2 = { 12, image.width, image.height, 1, 1 };
    fwrite(&header2, 1, sizeof(bitmap2header), file);
    /* Go down the rows and write pixels - with padding */
    uint32_t y;
    for (y = 0; y < image.height; ++y)
...
{   /* Padded width for 16 bit bitmap in pixels(2 bytes each) */
    ...
    for (y = 0; y < image.height; ++y)
    {   /* Write row by row with 4 byte padding */
        fwrite(&image16bit[y * paddedwidth], paddedwidth, sizeof(uint16_t), file);
    }
    fclose(file);
}
...
{   /* Open file */
    ...
    imagestruct image = { imagewidth, imageheight, imagedata, 0, 0 };
    return image;
}